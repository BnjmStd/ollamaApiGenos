...
# Importaciones del Chat y otros módulos
from chat.routes import router as chat_router
from chat.routes import manager as chat_manager
...
async def health_check():
    ...
    return {
        "status": "healthy",
        "websocket_connections": len(chat_manager.active_connections),
        "system": {
...
async def get_metrics():
    ...
    try:
        ollama_metrics = await ollama_service.get_metrics()
        return {
            "timestamp": time.time(),
            "system": {
...
def find_unit_in_line(line: str) -> Optional[str]:
    """Busca la unidad en una línea específica."""
...
def find_method_in_line(line: str) -> Optional[str]:
    """Busca el método en una línea específica."""
...
def save_exam_results_to_file(single_exam: SingleExam, patient_data: dict, base_filename: str):
    """Guarda los resultados de un examen en un archivo JSON."""
...
async def process_exam_section(
    exam: dict,
    section_text: str,
    patient_data: dict,
    base_filename: str
) -> Optional[SingleExam]:
    """Procesa un examen detectado y sus componentes."""
    try:
        exam_name = exam["name"]
...
# Nueva función de validación para el contenido base64
def validate_pdf_content(pdf_data: PDFContent) -> bool:
    """Valida el contenido del JSON y el PDF en base64"""
    try:
...
async def classify_exam(
    ...
    try:
        ...
        # Crear un objeto UploadFile para mantener compatibilidad
        file = UploadFile(
        ...
        # Agregar metadata del JSON original
...
async def get_rate_limit_status(request: Request):
    """Endpoint para verificar el estado del rate limiting"""
    ip = request.client.host
    current_time = time.time()
    
    # Calcular requests en diferentes ventanas de tiempo
